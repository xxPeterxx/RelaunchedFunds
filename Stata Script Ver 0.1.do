...
*S2.2 Check duration gaps using a complex loop
			...
			*matrix list mat_date 
			...
			forvalues i = 1/`r' { 
				...
				forvalues k = 1/`r' {	// 3 is the last row
					if `i'~=`k' {
						*disp "i is `i', k is `k'"
						if (mat_date[`i',1]>mat_date[`k',1] & mat_date[`i',1]>mat_date[`k',2]) {
							*disp "INC `i' > INC`k'; END`i' > END`k'"
							if is_gap==-99 {
								scalar is_gap = 1
							}
							*disp "!! 1) is_gap is:" is_gap
						}
	...
	tab gap
	order PersonID ProductReference gap InceptionDate PerformanceEndDate first_end_date
	...
	order PersonID ProductReference gap InceptionDate PerformanceEndDate mean*
